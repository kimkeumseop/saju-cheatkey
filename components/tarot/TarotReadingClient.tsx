...
const T = {
  ...
  chip: { background: 'rgba(124,111,214,0.12)', border: '1px solid rgba(124,111,214,0.25)', color: '#7c6fd6' } as React.CSSProperties,
  dot: (active: boolean, passed: boolean) => ({
    background: active ? '#7c6fd6' : passed ? 'rgba(124,111,214,0.4)' : 'rgba(45,27,30,0.12)',
    ...
    <main className="relative min-h-screen overflow-x-hidden pb-20" style={{ background: '#FBF7F2' }}>
      ...
      <div className="relative z-10 max-w-lg mx-auto px-4 pt-8">
        ...
        <div className="text-center mb-8">
          <div className="text-xs font-black tracking-widest mb-2" style={{ color: '#7c6fd6' }}>✦ TAROT READING ✦</div>
          <h1 className="text-2xl font-bold" style={{ color: '#2D1B1E', fontFamily: '"Noto Serif KR", serif' }}>타로 리딩</h1>
          <p className="text-sm mt-1" style={{ color: 'rgba(45,27,30,0.42)' }}>마음속 질문을 품고 카드를 선택해요</p>
        </div>
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              ...
              />
              {i < 4 && <div className="w-6 h-px bg-black/10" />}
            </div>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="spread" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}>
              ...
              <div className="space-y-3">
                {SPREADS.map((s) => (
                  ...
                  >
                    <div className="font-bold text-[#2D1B1E] text-sm">{s.label}</div>
          ...
          {step === 'question' && (
            <motion.div key="question" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}>
              ...
              <p className="text-xs text-black/40 text-center mb-5">질문이 없으면 넘어가도 괜찮아요 ✨</p>
              <textarea
                value={question}
                onChange={(e) => setQuestion(e.target.value)}
                placeholder="예) 이 연애, 계속 이어나가야 할까요?"
                className="w-full h-28 p-4 rounded-2xl text-sm resize-none outline-none transition-all text-[#2D1B1E] placeholder:text-black/30"
              ...
              <div className="flex gap-3 mt-4">
                <button onClick={() => setStep('spread')} className="flex-1 py-3.5 rounded-2xl border border-black/15 text-black/50 text-sm hover:bg-black/5 transition-colors">
                  ← 뒤로
                </button>
          ...
          {step === 'shuffle' && (
            <motion.div key="shuffle" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="text-center">
              ...
              <p className="text-xs text-black/40 mb-8">덱을 탭하여 셔플해요</p>
                      ...
                      <div className="text-center" style={{ color: 'rgba(255,255,255,0.75)' }}>
                        {shuffling
                          ? <div className="w-8 h-8 border-2 rounded-full animate-spin mx-auto"
                              style={{ borderColor: 'rgba(45,27,30,0.3)', borderTopColor: 'white' }} />
              ...
              <p className="text-xs text-black/40">{shuffling ? '셔플 중...' : '덱을 탭하면 카드가 섞여요'}</p>
            </motion.div>
          )}
                ...
                {drawnCards.map((card, i) => {
                  ...
                  return (
                    <div key={i} className="flex flex-col items-center gap-2">
                      ...
                      <div className="text-[10px] text-black/40 text-center">{card.position}</div>
                    </div>
                  )
                })}
              </div>
