...
export async function POST(req: Request) {
  ...
  try {
    ...
    const prompt = `
      ...
      [필수 지시사항 - 구조와 형식]
      1. 반드시 아래 4개의 주제를 모두 포함하여 최소 4개 이상의 독립된 분석 섹션을 작성해. AI가 임의로 답변을 중간에 끊거나 1개 섹션만 작성하는 것은 절대 금지.
         ① 타고난 기질과 본성
         ② 재물과 일의 흐름
         ③ 대인관계와 인연
         ④ 운명을 개척하기 위한 따뜻한 조언
      2. **각 주제(섹션)로 넘어갈 때마다 우리가 프론트엔드에서 카드를 쪼개는 기준점인 "## [이모지] [주제에 맞는 다정한 소제목]" 포맷을 절대 빼먹지 말고 매번 반복해서 작성해!**
         - 예시 1:
