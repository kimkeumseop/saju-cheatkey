...
export async function POST(req: Request) {
  ...
  try {
    ...
    const prompt = `
      ...
      [필수 지시사항]
      1. 말투: "~해요", "~군요", "~일 거예요" 같은 우아하고 다정한 경어체. 마치 곁에서 속삭여주는 느낌으로.
      2. 금기: 한자 절대 금지. 100% 한글만 사용.
      3. 톤: 단순히 길흉을 말하지 말고, 유저의 마음을 위로하고 설레게 만드는 '간질간질한' 표현을 듬뿍 사용해줘.
      4. 형식: 아래 JSON 구조로만 응답해.

      {
        "vibe": "오늘 당신을 감싸는 신비로운 기운 (시적인 표현)",
        "heartFlutter": "오늘 당신의 가슴을 뛰게 할 작은 순간 하나 (간질간질한 묘사)",
        "luckyWhisper": "행운을 부르는 당신만의 특별한 행동이나 마음가짐",
        "gift": "오늘 우주가 당신에게 주는 작은 선물 (컬러, 장소 등)"
      }
    `;
