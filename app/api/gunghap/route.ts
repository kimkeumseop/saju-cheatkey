...
export async function POST(req: Request) {
  ...
  try {
    ...
    const prompt = `
      ...
      [상대방: ${user2.name}] 사주: ${pillars2}

      [필수 지시사항]
      1. 궁합 점수: 0부터 100 사이의 정수(compatibilityScore)를 산출해.
      2. 분량 조절: 각 분석 섹션은 **250자 내외**로 풍성하게 작성해줘. 핵심 통찰을 시적인 문장으로 풀어내는 게 중요해.
      3. 말투: 우아하고 다정한 경어체 (~해요, ~군요).
      4. 형식: 아래 JSON 구조로만 응답해. 한자 절대 금지.
