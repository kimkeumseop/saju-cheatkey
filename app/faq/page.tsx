    ...
    <main className="relative min-h-screen overflow-x-hidden pt-24 pb-32" style={{ background: '#FBF7F2' }}>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 space-y-8">
        <header className="rounded-[3rem] p-8 md:p-12 space-y-4" style={{ background: 'rgba(255,255,255,0.92)', border: '1px solid rgba(212,104,138,0.14)', boxShadow: '0 8px 40px rgba(212,104,138,0.06)' }}>
          <p className="text-xs font-black tracking-[0.25em] uppercase" style={{ color: 'rgba(212,104,138,0.7)' }}>FAQ</p>
          <h1 className="text-3xl md:text-5xl font-bold tracking-tight" style={{ color: '#2D1B1E', fontFamily: '"Noto Serif KR", serif' }}>자주 묻는 질문</h1>
          <p className="leading-relaxed break-keep" style={{ color: 'rgba(45,27,30,0.6)' }}>
        ...
        <section className="space-y-4">
          {faqEntries.map((entry, index) => (
            <article key={entry.question} className="rounded-[2rem] p-6 md:p-8 space-y-3" style={{ background: 'rgba(255,255,255,0.92)', border: '1px solid rgba(212,104,138,0.12)' }}>
              <p className="text-xs font-black tracking-[0.2em] uppercase" style={{ color: 'rgba(212,104,138,0.7)' }}>Q{index + 1}</p>
              <h2 className="text-xl font-bold break-keep" style={{ color: '#2D1B1E' }}>{entry.question}</h2>
              <p className="leading-8 break-keep" style={{ color: 'rgba(45,27,30,0.6)' }}>{entry.answer}</p>
            </article>
          ))}
        </section>
        ...
        <section className="rounded-[2.5rem] p-8 md:p-10 space-y-4" style={{ background: 'radial-gradient(ellipse at 0% 0%, rgba(124,111,214,0.12), rgba(255,255,255,0.85) 60%)', border: '1px solid rgba(124,111,214,0.16)' }}>
          <h2 className="text-2xl font-bold" style={{ color: '#2D1B1E' }}>추가 문의 안내</h2>
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/contact" className="px-4 py-2 rounded-full font-bold text-white" style={{ background: 'linear-gradient(135deg, #d4688a, #c2255c)' }}>
              문의하기
            </Link>
