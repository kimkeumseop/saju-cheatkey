...
import { Loader2, Sparkles, Layout, Lock, Zap, ChevronRight, AlertCircle, Heart, Coins, BarChart3, Star, History, Target, Fingerprint } from 'lucide-react';
          ...
          return (
            <div key={colIdx} className="space-y-3">
              <div className="text-center space-y-1">
                <div className="text-[10px] font-black text-gray-400 tracking-tighter opacity-80 uppercase">{p.label}</div>
...
function MbtiGauge({ mbti }: { mbti: string }) {
  return (
    <div className="bg-white p-8 rounded-[2.5rem] shadow-lg border border-gray-100 space-y-6 overflow-hidden relative">
      ...
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-xl font-black text-[#3C1E1E]">MBTI 싱크로율</h3>
      </div>

      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <p className="text-sm font-bold text-gray-400 tracking-widest">타고난 본질 성향</p>
          <p className="text-4xl font-black text-indigo-600 tracking-tighter">{mbti}</p>
        </div>
        <div className="text-right">
          <p className="text-[10px] font-black text-gray-300 uppercase">Sync Rate</p>
          <p className="text-3xl font-black text-[#3C1E1E]">98.2%</p>
        </div>
      </div>

      <div className="h-4 w-full bg-gray-50 rounded-full overflow-hidden border border-gray-100 p-1">
        <div className="h-full bg-gradient-to-r from-indigo-400 to-indigo-600 rounded-full w-[98%] animate-pulse" />
      </div>
      
      <p className="text-[11px] text-gray-400 font-bold leading-relaxed">
        * 당신의 사주 오행 분포와 십신 데이터를 분석한 결과, 사회적 가면(페르소나) 뒤에 숨겨진 실제 기질이 {mbti}와 완벽하게 일치합니다.
      </p>
    </div>
  );
}
...
export default function SajuResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#F7F8FA] flex flex-col items-center justify-center p-6 text-center">
      ...
      <p className="mt-4 text-gray-500 font-bold">인생 치트키 분석 중...</p>
    </div>
  );
          ...
          <div className="space-y-12">
            <div className="text-center space-y-6">
              <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-[#FEE500] text-[#3C1E1E] text-sm font-bold shadow-sm">
                ...
                전문가급 명리학 x MBTI 해부 리포트
              </div>
              <h1 className="text-4xl md:text-5xl font-bold text-gray-900 tracking-tight">
                <span className="text-[#3C1E1E] bg-[#FEE500] px-2 rounded-lg">{data.userName}</span> 님의 분석
              </h1>
            </div>
            ...
            <div className="bg-white p-6 md:p-10 rounded-[3rem] shadow-xl border border-gray-100 space-y-10 relative overflow-hidden">
              ...
              <div className="text-center space-y-1 relative z-10"><h3 className="text-2xl font-black text-gray-900 tracking-tight">인생 설계도 (만세력)</h3><p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.3em]">Manseyrok INFOGRAPHIC</p></div>

              <PillarChart pillars={saju.pillars} />

              <div className="pt-8 border-t border-gray-50 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <div className="flex items-center gap-2 ml-1"><BarChart3 className="w-4 h-4 text-indigo-500" /><span className="text-sm font-black text-[#3C1E1E]">오행 분포</span></div>
                  <ElementsChart counts={saju.elementsCount} />
                </div>
                <div className="space-y-4">
                  <div className="flex items-center gap-2 ml-1"><Star className="w-4 h-4 text-amber-500" /><span className="text-sm font-black text-[#3C1E1E]">핵심 귀인 & 신살</span></div>
                  <div className="flex flex-wrap gap-2">
                    {saju.keyShinsal.map((s: string) => <div key={s} className="px-4 py-2 rounded-2xl bg-indigo-50 text-indigo-600 text-[11px] font-black border border-indigo-100 shadow-sm">#{s}</div>)}
                  </div>
                </div>
              </div>
            </div>

            <MbtiGauge mbti={saju.mbti} />

            <div className="bg-white p-8 md:p-10 rounded-[3rem] shadow-lg border border-gray-100 space-y-8">
              <div className="flex items-center gap-2"><History className="w-5 h-5 text-indigo-500" /><h3 className="text-xl font-black text-[#3C1E1E]">대운 (10년 주기의 흐름)</h3></div>
            ...
            <div className="space-y-8 relative">
              <div className="flex items-center gap-3 ml-2"><Layout className="w-6 h-6 text-[#3C1E1E]" /><h3 className="text-2xl font-bold text-gray-900">심층 분석 리포트</h3></div>

              {isUnlocked ? (
                <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000">
                  {(() => {
                    const themes = [
                      { key: 'headline', theme: '운명의 헤드라인', title: '뼈 때리는 요약', icon: '📢' },
                      { key: 'mbtiAnalysis', theme: '사주 x MBTI 해부', title: '성격의 뿌리', icon: '👤' },
                      { key: 'wealthGrade', theme: '자본주의 생존 등급', title: '재물 그릇', icon: '💰' },
                      { key: 'careerStrategy', theme: '직업 팩폭 전략', title: '돈 냄새 맡는 커리어', icon: '💼' },
                      { key: 'relationshipList', theme: '인간관계 살생부', title: '손절과 귀인', icon: '💀' },
                      { key: 'selfDestruct', theme: '내 발등 찍는 버튼', title: '무의식적 오류', icon: '⚠️' },
                      { key: 'cheatKey2026', theme: '2026 실전 치트키', title: '월별 행동 강령', icon: '🗓️' }
                    ];
                    const displayData = themes.map(t => ({
                      theme: t.theme, title: t.title, icon: t.icon, content: data.aiResult[t.key] || '분석 내용을 불러오지 못했습니다.'
                    }));
                ...
                <div className="bg-white rounded-[3rem] p-8 md:p-12 shadow-xl border border-gray-100 space-y-10 relative overflow-hidden">
                  <div className="flex items-center justify-between bg-gray-50 p-6 rounded-3xl border border-gray-100">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center"><Coins className="w-5 h-5 text-amber-500" /></div>
                      <div><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">보유 치트키</p><p className="text-lg font-black text-[#3C1E1E]">{userCheatKeys} 개</p></div>
                    </div>
                    <div className="h-10 w-px bg-gray-200" /><div className="text-right"><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">필요 치트키</p><p className="text-lg font-black text-[#3C1E1E]">1 개</p></div>
                  </div>
                  <div className="space-y-4 text-center sm:text-left">
                    <div className="flex items-center justify-center sm:justify-start gap-2 text-red-500 font-black text-sm"><AlertCircle className="w-4 h-4" /><span>충격 주의: 당신의 진짜 모습은 이게 아닙니다</span></div>
                    <h4 className="text-2xl md:text-3xl font-black text-[#3C1E1E] leading-tight">"{data.userName}님, 사실 <span className="text-red-500 underline underline-offset-4">숨겨진 대박 기회</span>가 따로 있다는 거 아시나요?"</h4>
                  </div>
                  <div className="bg-[#3C1E1E] rounded-[2.5rem] p-8 text-center shadow-2xl relative overflow-hidden group">
                    <div className="space-y-6 relative z-10">
                      <div className="flex items-center justify-center gap-2 text-[#FEE500]"><Zap className="w-6 h-6 fill-[#FEE500]" /><span className="font-black text-xs tracking-widest uppercase italic">Destiny CheatKey</span></div>
                      <h2 className="text-white text-2xl md:text-3xl font-black leading-tight"><span className="text-[#FEE500]">777원</span>으로 <br/>인생의 치트키를 켜세요!</h2>
                      <button onClick={() => setShowPaymentWidget(true)} className="w-full bg-[#FEE500] hover:bg-[#FDD000] text-[#3C1E1E] py-6 rounded-2xl font-black text-xl shadow-lg transition-all active:scale-[0.95] flex items-center justify-center gap-3">777원 충전하기 (결제)<ChevronRight className="w-6 h-6" /></button>
                    </div>
                  </div>
                  <div className="space-y-6 opacity-40 select-none pointer-events-none filter blur-[8px]"><div className="h-4 bg-gray-100 rounded-full w-full" /><div className="h-4 bg-gray-100 rounded-full w-5/6" /><div className="h-4 bg-gray-100 rounded-full w-4/6" /></div>
                </div>
              )}

              {showPaymentWidget && <PaymentWidget resultId={id} onCancel={() => setShowPaymentWidget(false)} />}
              <div className="mt-12 bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm text-center"><ShareButtons name={data.userName} /></div>
            </div>
          </div>
        )}
      </div>
