    ...
    <main className="relative min-h-screen overflow-x-hidden pt-24 pb-32" style={{ background: '#0d0710' }}>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 space-y-8">
        <header className="rounded-[3rem] p-8 md:p-12 space-y-4" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(232,130,154,0.14)', boxShadow: '0 8px 40px rgba(232,130,154,0.06)' }}>
          <p className="text-xs font-black tracking-[0.25em] uppercase" style={{ color: 'rgba(232,130,154,0.7)' }}>About</p>
          <h1 className="text-3xl md:text-5xl font-bold tracking-tight" style={{ color: '#f5eef2', fontFamily: '"Noto Serif KR", serif' }}>{SITE_NAME} 소개</h1>
        ...
        <section className="grid gap-5 md:grid-cols-2">
          <article className="rounded-[2rem] p-6 md:p-8 space-y-3" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(232,130,154,0.12)' }}>
            <h2 className="text-xl font-bold text-[#f5eef2]">무엇을 제공하나요</h2>
            <p className="leading-8 break-keep" style={{ color: 'rgba(240,232,238,0.6)' }}>
              사주 기초 개념, 오행과 천간지지 설명, 운세 해석 참고 가이드, 자주 묻는 질문, 개인정보 및 이용 정책을 함께 제공합니다. 결과를 읽는 과정에서 생기는 질문도 내부 링크를 통해 바로 확인할 수 있습니다.
            </p>
          </article>
          <article className="rounded-[2rem] p-6 md:p-8 space-y-3" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(232,130,154,0.12)' }}>
            <h2 className="text-xl font-bold text-[#f5eef2]">어떤 톤을 지향하나요</h2>
            <p className="leading-8 break-keep" style={{ color: 'rgba(240,232,238,0.6)' }}>
              과장된 예언이나 자극적인 표현보다, 참고용 정보와 자기 이해를 돕는 설명형 톤을 지향합니다. 사주를 처음 접하는 이용자도 부담 없이 읽을 수 있게 용어를 풀어쓰는 방식을 우선합니다.
            </p>
          </article>
          <article className="rounded-[2rem] p-6 md:p-8 space-y-3" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(232,130,154,0.12)' }}>
            <h2 className="text-xl font-bold text-[#f5eef2]">운영 정보</h2>
            <p className="leading-8 break-keep" style={{ color: 'rgba(240,232,238,0.6)' }}>
              운영 주체는 {SITE_TEAM}이며, 사이트 전반의 콘텐츠와 정책 문서는 동일한 기준으로 관리합니다. 문의와 정책 관련 안내는 전용 페이지와 푸터 링크에서 확인할 수 있습니다.
            </p>
          </article>
          <article className="rounded-[2rem] p-6 md:p-8 space-y-3" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(232,130,154,0.12)' }}>
            <h2 className="text-xl font-bold text-[#f5eef2]">문의 및 확인</h2>
            <p className="leading-8 break-keep" style={{ color: 'rgba(240,232,238,0.6)' }}>
        ...
        <section className="rounded-[2.5rem] p-8 md:p-10 space-y-4" style={{ background: 'radial-gradient(ellipse at 0% 0%, rgba(157,143,255,0.12), rgba(18,8,16,0.85) 60%)', border: '1px solid rgba(157,143,255,0.16)' }}>
          <h2 className="text-2xl font-bold" style={{ color: '#f5eef2' }}>함께 읽으면 좋은 페이지</h2>
          <div className="flex flex-wrap gap-3">
            <Link href="/guide/what-is-saju" className="px-4 py-2 rounded-full font-bold text-white" style={{ background: 'linear-gradient(135deg, #e8829a, #c2255c)' }}>
              사주란 무엇인가
            </Link>
            <Link href="/faq" className="px-4 py-2 rounded-full font-bold" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(157,143,255,0.2)', color: 'rgba(240,232,238,0.82)' }}>
              FAQ
            </Link>
            <Link href="/contact" className="px-4 py-2 rounded-full font-bold" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(157,143,255,0.2)', color: 'rgba(240,232,238,0.82)' }}>
              문의하기
            </Link>
          </div>
        </section>
